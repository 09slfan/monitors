...
// | Author: 小艾.pino
// +----------------------------------------------------------------------
namespace app\food\taglib;

use think\template\TagLib;

class Food extends TagLib
{
    /**
     * 定义标签列表
     */
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\food\service\ApiService::articles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagTagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\food\service\ApiService::tagArticles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagPage($tag, $content)
    {
        ...
        $returnVarName = empty($tag['item']) ? 'food_page' : $tag['item'];

        $parse = <<<parse
<?php
\${$returnVarName} = \app\food\service\ApiService::page({$id});
?>
{$content}
parse;
        return $parse;
...
<?php
if(!empty({$cid})){
    \$__BREADCRUMB_ITEMS__ = \app\food\service\ApiService::breadcrumb({$cid},{$self});
?>
...
parse;
    ...
    public function tagCategories($tag, $content)
    {
        ...
        $returnVarName = 'food_categories_data';
        if (strpos($ids, '$') === 0) {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\food\service\ApiService::categories([
    'where'   => {$where},
    'order'   => '{$order}',
    'ids'     => {$ids}
]);
...
parse;
    ...
    public function tagCategory($tag, $content)
    {
        ...
        $returnVarName = empty($tag['item']) ? 'food_category' : $tag['item'];

        $parse = <<<parse
<?php
\${$returnVarName} = \app\food\service\ApiService::category({$id});
?>
{$content}
parse;
        return $parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'food_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\food\service\ApiService::subCategories({$categoryId});
...
parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'food_all_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\food\service\ApiService::allSubCategories({$categoryId});
 ?>
<volist name="{$returnVarName}" id="{$item}">
{$content}
</volist>
