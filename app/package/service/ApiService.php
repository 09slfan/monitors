...
class ApiService
{
    /**
     * 功能:查询食品库列表,支持分页;<br>
     * 注:此方法查询时关联两个表package_category_post(category_post),package_post(post);在指定排序(order),指定查询条件(where)最好指定一下表别名
     ...
     *                     )
     *                     字段说明:
     *                     category_ids:食品库所在分类,可指定一个或多个分类id,以英文逗号分隔,如1或1,2,3 默认值为全部
     *                     field:调用指定的字段@todo
     *                     如只调用posts表里的id和post_title字段可以是post.id,post.post_title; 默认全部,
     *                     此方法查询时关联两个表package_category_post(category_post),package_post(post);
     *                     所以最好指定一下表名,以防字段冲突
     ...
     *                     </pre>
     * @return array 包括分页的食品库列表<pre>
     *                     格式:
     *                     array(
     *                     "articles"=>array(),//食品库列表,array
     *                     "page"=>"",//生成的分页html,不分页则没有此项
     *                     "total"=>100, //符合条件的食品库总数,不分页则没有此项
    ...
    public static function articles($param)
    {
        ...
        if (!empty($categoryIds)) {
            ...
            array_push($join, ['__PACKAGE_CATEGORY_POST__ category_post', 'post.id = category_post.post_id']);
        ...
        $articles = $packagePostModel->alias('post')->field($field)
            ->join($join)
            ->where($where)
            ->where($paramWhere)
    ...
    /**
     * 功能:查询标签食品库列表,支持分页;<br>
     * 注:此方法查询时关联两个表package_tag_post(tag_post),package_post(post);在指定排序(order),指定查询条件(where)最好指定一下表别名
     * @param array $param 查询参数<pre>
     *                     array(
     *                     'tag_id'=>'',
     *                     'where'=>'',
     ...
     *                     field:调用指定的字段@todo
     *                     如只调用posts表里的id和post_title字段可以是post.id,post.post_title; 默认全部,
     *                     此方法查询时关联两个表package_tag_post(category_post),package_post(post);
     *                     所以最好指定一下表名,以防字段冲突
     *                     limit:数据条数,默认值为10,可以指定从第几条开始,如3,8(表示共调用8条,从第3条开始)
     *                     order:排序方式,如按posts表里的published_time字段倒序排列：post.published_time desc
     *                     where:查询条件,字符串形式,和sql语句一样,请在事先做好安全过滤,最好使用第二个参数$where的数组形式进行过滤,此方法查询时关联多个表,所以最好指定一下表名,以防字段冲突,查询条件(只支持数组),格式和thinkPHP的where方法一样,此方法查询时关联多个表,所以最好指定一下表名,以防字段冲突;
    ...
    public static function tagArticles($param)
    {
        ...
        if (empty($tagId)) {
            ...
            array_push($join, ['__PACKAGE_TAG_POST__ tag_post', 'post.id = tag_post.post_id']);
    ...
    public static function article($id)
    {
        ...
        return $packagePostModel->where($where)
    ...
    public static function category($id)
    {
        ...
        return $packageCategoryModel->where($where)->find();
    }
    ...
    public static function subCategories($categoryId, $field = '*')
    {
		...
		return $packageCategoryModel->field($field)->where($where)->order('list_order ASC')->select();
	}
    ...
    public static function allSubCategories($categoryId)
    {
        ...
        if ($categoryId !== 0) {
            $category = $packageCategoryModel->field('path')->where('id', $categoryId)->find();
        ...
        return $packageCategoryModel->where($where)->whereLike('path', "$categoryPath-%")->select();
    }
    ...
    /**
     * 获取面包屑数据
     * @param int     $categoryId  当前食品库所在分类,或者当前分类的id
    ...
    public static function breadcrumb($categoryId, $withCurrent = false)
    {
        ...
        $path = $packageCategoryModel->where(['id' => $categoryId])->value('path');
        ...
        if (!empty($path)) {
            ...
            if (!empty($parents)) {
                $data = $packageCategoryModel->where('id', 'in', $parents)->order('path ASC')->select();
            }
        }
